...
export default function CampusesPage() {
    ...
    return (
        <div className="space-y-6">
            ...
            <form onSubmit={submit} className="grid gap-3 rounded-2xl border bg-white p-5 shadow-sm lg:grid-cols-[1fr_180px_1.5fr_auto]">
                <input className="rounded-xl border border-slate-200 px-3 py-2" placeholder="Nombre de sede" value={form.name} onChange={(event) => setForm({ ...form, name: event.target.value })} required />
                <input className="rounded-xl border border-slate-200 px-3 py-2" placeholder="Ciudad" value={form.city} onChange={(event) => setForm({ ...form, city: event.target.value })} required />
                <input className="rounded-xl border border-slate-200 px-3 py-2" placeholder="Direccion opcional" value={form.address} onChange={(event) => setForm({ ...form, address: event.target.value })} />
            ...
            {!loading && !error && items.length > 0 && (
                <div className="grid gap-4 lg:grid-cols-2">
                    {items.map((item) => <article key={item.id} className="rounded-2xl border bg-white p-5 shadow-sm">
                        <div className="flex items-start justify-between gap-4"><div className="flex gap-3"><div className="rounded-xl bg-orange-50 p-3 text-orange-600"><MapPin size={20} /></div><div><h2 className="font-bold text-slate-900">{item.name}</h2><p className="text-sm text-slate-500">{item.city}</p><p className="mt-1 text-xs text-slate-400">{item.address || 'Direccion pendiente de registro'}</p></div></div><StatusBadge value={item.is_active ? 'ACTIVE' : 'INACTIVE'} /></div>
                        <div className="mt-4 flex justify-end gap-2 border-t pt-3"><button onClick={() => { setEditingId(item.id); setForm({ name: item.name, city: item.city, address: item.address || '', is_active: item.is_active }) }} className="rounded-lg p-2 text-blue-600 hover:bg-blue-50" title="Editar sede"><Pencil size={16} /></button><button onClick={() => remove(item.id)} className="rounded-lg p-2 text-red-600 hover:bg-red-50" title="Eliminar sede"><Trash2 size={16} /></button></div>
                    </article>)}
                </div>
            )}
        </div>
