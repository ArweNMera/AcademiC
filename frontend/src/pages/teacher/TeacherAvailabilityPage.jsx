...
export default function TeacherAvailabilityPage() {
    ...
    useEffect(() => { load().catch(() => toast.error('No se pudo cargar la disponibilidad.')) }, [])
    ...
    return <div className="space-y-5"><Header title="Mi disponibilidad" text="Administra solamente tus propias franjas disponibles." />
        <form onSubmit={submit} className="grid gap-3 rounded-2xl border bg-white p-5 md:grid-cols-5">
            <select className="rounded-xl border p-3" value={form.day_of_week} onChange={(e) => setForm({ ...form, day_of_week: Number(e.target.value) })}>{['Lunes', 'Martes', 'Miercoles', 'Jueves', 'Viernes', 'Sabado', 'Domingo'].map((day, index) => <option key={day} value={index + 1}>{day}</option>)}</select>
            <input className="rounded-xl border p-3" type="time" value={form.start_time} onChange={(e) => setForm({ ...form, start_time: e.target.value })} />
            <input className="rounded-xl border p-3" type="time" value={form.end_time} onChange={(e) => setForm({ ...form, end_time: e.target.value })} />
        ...
        <section className="space-y-3">{items.map((item) => <article key={item.id} className="flex justify-between rounded-2xl border bg-white p-4"><p><strong>Dia {item.day_of_week}</strong> | {short(item.start_time)} - {short(item.end_time)} | {item.is_available ? 'Disponible' : 'No disponible'}</p><div className="flex gap-2"><button onClick={() => { setEditingId(item.id); setForm({ day_of_week: item.day_of_week, start_time: short(item.start_time), end_time: short(item.end_time), is_available: item.is_available }) }} className="rounded-lg border p-2"><Pencil size={16} /></button><button onClick={async () => { await teacherAvailabilityService.deleteMine(item.id); await load() }} className="rounded-lg border p-2 text-red-600"><Trash2 size={16} /></button></div></article>)}</section>
    </div>
}
