...
export default function TeacherChangeRequestsPage() {
    ...
    const load = async () => { setItems(await scheduleChangeRequestService.listMine()); const schedule = await teacherPortalService.getSchedule(); setBlocks(schedule.blocks || []) }
    ...
    return <div className="space-y-5"><Header title="Solicitudes de cambio" text="La coordinacion revisa tu solicitud; no modifica automaticamente el horario." />
        ...
        <section className="space-y-3">{items.map((item) => <article key={item.id} className="rounded-2xl border bg-white p-4"><div className="flex justify-between"><p className="font-bold">{item.request_type} - {item.course_name || 'General'}</p><span>{item.status}</span></div><p className="text-sm text-slate-600">{item.reason}</p>{item.coordinator_response && <p className="mt-2 text-sm text-blue-700">Respuesta: {item.coordinator_response}</p>}{item.status === 'PENDING' && <button className="mt-3 rounded-lg border px-3 py-2 text-sm font-bold text-red-600" onClick={async () => { await scheduleChangeRequestService.cancelMine(item.id); await load() }}>Cancelar</button>}</article>)}</section>
    </div>
}
