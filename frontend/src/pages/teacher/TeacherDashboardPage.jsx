...
import { AlertTriangle, BookOpen, CalendarDays, Clock, Loader2, MessageSquare } from 'lucide-react'
...
export default function TeacherDashboardPage() {
    ...
    useEffect(() => {
        teacherPortalService.getDashboard().then(setData).catch(() => toast.error('No se pudo cargar el panel docente.'))
    }, [])
    ...
    return <div className="space-y-6">
        ...
        <section className="rounded-2xl border bg-white p-5">
            ...
            {data.upcoming_classes.map((item) => <p key={item.schedule_block_id} className="border-t py-3 text-sm"><strong>{item.course_name}</strong> - Seccion {item.section_code} | Dia {item.day_of_week}, {shortTime(item.start_time)} - {shortTime(item.end_time)} | {item.classroom || 'Virtual'}</p>)}
            {!data.upcoming_classes.length && <p className="text-slate-500">No existe horario publicado asignado.</p>}
        </section>
    </div>
}
function Loading() { return <div className="flex justify-center p-12"><Loader2 className="animate-spin" /></div> }
